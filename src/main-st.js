...
// ─────────────────────────────────────────────────────────────────────────────
// Section-triggered media scrubbing (ScrollTrigger "hooks" model)
// ─────────────────────────────────────────────────────────────────────────────
const SECTION_SCRUB_START = 'top top'
const SECTION_SCRUB_END = 'bottom top'
const SECTION_SCRUB_SMOOTH = 0.6
const DESKTOP_STATS_IN = 0.835
const clamp01 = (v) => Math.max(0, Math.min(1, v))

const pickSection = (...selectors) => document.querySelector(selectors.join(', '))

function createSectionScrub({ label, section, onProgress }) {
  if (!section || !ScrollTrigger) {
    console.warn(`[${label}] missing section or ScrollTrigger`, { section, ScrollTrigger })
    return null
  }

  return ScrollTrigger.create({
    trigger: section,
    start: SECTION_SCRUB_START,
    end: SECTION_SCRUB_END,
    scrub: SECTION_SCRUB_SMOOTH,
    onUpdate: (self) => onProgress(clamp01(self.progress), self),
  })
}
...
function setupStatsSectionLottie() {
  ...
  getWebflowLottie('.stats-arc').then((anim) => {
    ...
    const seek = (progress) => {
      const frame = Math.round(ip + clamp01(progress) * totalFrames)
    ...
    sectionTrigger = createSectionScrub({
      label: 'Stats Lottie',
      section: pickSection('.cc-stats'),
      onProgress: (progress) => {
        if (!hasResized && progress > 0) {
          hasResized = true
          if (typeof anim.resize === 'function') anim.resize()
        }
        seek(progress)
...
function setupConvergeSectionLottie() {
  ...
  getWebflowLottie('.convergence-lottie').then((anim) => {
    ...
    sectionTrigger = createSectionScrub({
      label: 'Convergence Lottie',
      section: pickSection('.cc-convergence', '.cc-benefits'),
      onProgress: (progress) => {
        if (!hasResized && progress > 0) {
          hasResized = true
          if (typeof anim.resize === 'function') anim.resize()
...
gltfLoader.load(
  ...
  (gltf) => {
    ...
    const hasStatsSection = Boolean(document.querySelector(selectors.panelStats))

    const desktopStatsIn = hasStatsSection ? DESKTOP_STATS_IN : 1
