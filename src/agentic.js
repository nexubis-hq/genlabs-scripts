...
if (window.__GENLABS_AGENTIC_BOOTED__) {
  throw new Error('GEN Labs agentic.js initialized more than once')
}
window.__GENLABS_AGENTIC_BOOTED__ = true
...
gltfLoader.load(
  ...
  (gltf) => {
    ...
    const TIMING = {
      ...
      // cc-convergence is excluded — all convergence timings set beyond range
      convergeIn: 2,
      convergeGlbStart: 2,
      centralizedIn: 2,
      convergeFadeIn: 0.06,
      ...
      modelFadeOutStart: 2,
      modelFadeOut: 0.03,
      // Stats appear immediately after the about section exits
      statsIn: isMobile() ? 2 : 0.52,
    }
    ...
    // ── Stats timeline tweens (desktop only) ──
    // cc-convergence is excluded — jump straight from the about section to stats.
    ...
    } // end if (!isMobile()) — stats timeline tweens
